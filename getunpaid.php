...
$count=0; $tot=0;
echo "
<form action='trans_payment.php' method='post'>
<table class='w3-table-all w3-card-4'>
";
//Records for Visits
$sqlv="SELECT t_visit.date, t_visit.time, doctor.d_name, t_visit.comment, t_bill.charge, t_bill.t_id FROM t_bill INNER JOIN t_visit ON t_bill.t_id = t_visit.v_id INNER JOIN doctor ON t_visit.d_id = doctor.d_id WHERE t_bill.p_id ='$pat' AND t_bill.status = 0 AND t_bill.t_type='V'" ;
...
while($rowv = mysqli_fetch_array($resv)) {
    echo "<tr>";
    	...
    	echo "<td><input type='checkbox' name='check_list_v[]' value=".$rowv['t_id']."></td>";
    echo "</tr>";
    $tot+=$rowv['charge'];
}
}
...
while($rowa = mysqli_fetch_array($resa)) {
    echo "<tr>";
    	...
    	echo "<td><input type='checkbox' name='check_list_a[]' value=".$rowa['t_id']."></td>";
    echo "</tr>";
    $tot+=$rowa['charge'];
}
}
...
while($rowc = mysqli_fetch_array($resc)) {
    echo "<tr>";
    	...
    	echo "<td><input type='checkbox' name='check_list_c[]' value=".$rowc['t_id']."></td>";
    echo "</tr>";
    $tot+=$rowc['charge'];
}
}
...
if($count==3)
{
	echo"
	<tr class='w3-red'>
		<th colspan='6'>No Unpaid Records</th>
	</tr>
	</table></form>";
}
else{
echo "
	<tr>
		<th colspan='5' style='text-align:right'>Total Amount Unpaid</th>
		<th>".$tot."</th>
	</tr>
</table>

<input class='submit' type='submit' name ='submit' style='float:right' value='Pay Checked'>
</form>";
}
mysqli_close($link);
?>
